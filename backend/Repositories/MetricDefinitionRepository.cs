...
public class MetricDefinitionRepository(LabInsightDbContext dbContext)
    : Repository<MetricDefinitionEntity>(dbContext), IMetricDefinitionRepository
{
    public async Task<IReadOnlyList<MetricDefinitionEntity>> ListOrderedByTechnicalNameAsync(
        bool isDeleted,
    ...
    public async Task<IReadOnlyDictionary<string, MetricDefinitionEntity>> GetByTechnicalNameAsync(
        CancellationToken cancellationToken)
    {
